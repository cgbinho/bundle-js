...
class File {
  ...
  getContentWithIncludes(existingfiles = [], ancestorincludes = []) {
    ...
    ancestorincludes.forEach((file) => {
      if (this.absolutefilepath == file.absolutefilepath) {
        throw new Error(
          "Ran into cyclical inclusion! Cannot INCLUDE file within itself: " +
    ...
    // Return contents with @INCLUDE statements replaced with the inclusions ( ExtendScript Import style)
    this._contentwithincludes = this._contentwithincludes.replace(
      ATINCLUDE_REGEX(),
      (m, p) => {
        let includedfile = this.getRelativeFile(p, existingfiles);
        return includedfile.getContentWithIncludes(
          existingfiles,
          ancestorincludes.concat([this])
        );
      }
    );
  ...
  getFinalContent() {
    if (this._contentwithincludes == null) {
      throw new Error(
        "An unknown error occured (_contentwithincludes was null)"
      );
    }
  ...
  static resolve(filepath, basedir = process.cwd()) {
    return path.normalize(path.resolve(resolve.sync(filepath, { basedir })));
  }
}
...
function toposortDependencies(entryfilepath) {
  ...
  function DFSToposort(root) {
    if (root.permmark) return;
    if (root.tempmark) throw new Error("Found a dependency cycle");
